 ...
 * @return float
 */
typedef float (*xqc_get_CCA_info_metric_cb)(xqc_stream_CCA_info_t *CCA_sample);

/**
 * @brief added by jndu for CCA switching. sum up metric
 */
typedef float (*xqc_sum_up_metric_cb)(float metric_cwnd_up, float metric_cwnd_down, float metric_ip_info);

/**
 * @brief added by jndu for CCA switching. get ip info metric during init CCA
 */
typedef float (*xqc_get_ip_info_metric_cb)(xqc_ip_CCA_info_t *CCA_info, int i);

/**
 * @brief added by jndu for CCA switching. free resource
 */
typedef void (*xqc_free_resource_metric_cb)();

/**
 * @brief added by jndu for CCA switching. set thres
 */
typedef float (*xqc_get_CCA_info_metric_thres_cb)(xqc_stream_CCA_info_t *CCA_sample);

/**
 * @brief added by jndu for CCA switching. notice that CS can evaluate (choosen)
 */
typedef int (*xqc_notice_metric_can_evaluate)(xqc_stream_CCA_info_t *CCA_sample);
...
typedef struct xqc_congestion_control_callback_s {
    ...
    /* added by jndu for CCA switching */
    void (*xqc_cong_ctl_on_lost_pcc)(void *cong_ctl, xqc_packet_out_t *po, xqc_usec_t now);

    /* added by jndu for CCA switching */
    void (*xqc_cong_ctl_on_ack_pcc)(void *cong_ctl, xqc_packet_out_t *po, xqc_usec_t now, uint64_t rtt);

    /* added by jndu for CCA switching */
    void (*xqc_cong_ctl_on_packet_sent)(void *cong_ctl, xqc_packet_out_t *po, xqc_usec_t now);

    /* added by jndu for CCA switching */
    void (*xqc_cong_ctl_destroy)(void *cong_ctl);

    /* added by jndu for CCA switching */
    void (*xqc_cong_ctl_set_rtt)(void *cong_ctl, uint64_t min_rtt, uint64_t avg_rtt);

    /* added by jndu for CCA switching */
    void (*xqc_cong_ctl_on_switching)(void *cong_ctl, xqc_bool_t is_better);
...
// XQC_EXPORT_PUBLIC_API extern xqc_cong_ctrl_callback_t xqc_pcc_proteus_cb;
XQC_EXPORT_PUBLIC_API extern xqc_cong_ctrl_callback_t xqc_coward_cb;
...
typedef struct xqc_conn_settings_s {
    ...
    */
    ...
    /* added by jndu for CCA switching */
    xqc_get_CCA_info_metric_cb  metric_cwnd_up_cb;
    xqc_get_CCA_info_metric_cb  metric_cwnd_down_cb;
    xqc_get_CCA_info_metric_thres_cb metric_cwnd_up_thres_cb;
    xqc_get_CCA_info_metric_thres_cb metric_cwnd_down_thres_cb;
    xqc_get_ip_info_metric_cb   metric_cwnd_ip_info_cb;
    xqc_sum_up_metric_cb        metric_cwnd_sum_up_cb;
    xqc_free_resource_metric_cb metric_free_resource_cb;
