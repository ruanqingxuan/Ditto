...
# self define, added by jndu
# header_file_path = "./header.json"
...
async def perform_http_request(
    ...
    # perform request
    logger.info("run perform_http_request")
    start = time.time()
    test_start = time.time_ns()
    if data is not None:
        logger.info("run perform_http_request|data is not None")
        ...
        logger.info("run perform_http_request|data is None")
        # with open(header_file_path, "r", encoding="utf-8") as file:
        #     headers = json.load(file)
        #     logger.info("run_perform_http_request|headers=%s", headers)
        http_events = await client.get(url, headers=data)
        # logger.info("run_perform_http_request|http_events=%s", http_events)
        method = "GET"
    elapsed = time.time() - start
    test_elapsed = (time.time_ns() - test_start) / 1e6  # 转换为毫秒
    # print speed
    octets = 0
    ...
    logger.info(
        "Response received for %s %s : %d bytes in %.2f s (%.3f Mbps)"
        % (method, urlparse(url).path, octets, elapsed, octets * 8 / elapsed / 1000000)
    )
    # added by qnwang to test AR
    # 解析 URL 以生成唯一的文件名
    parsed_url = urlparse(url)
    domain_mapping = {
        "udpcc-shh2.dfshan.net_8000": "xquic",
        "udpcc.dfshan.net_8443": "ditto",
    }
    domain = parsed_url.netloc.replace(":", "_")  # 替换端口冒号
    # 根据不同的 domain 赋予不同的文件名
    filename = domain_mapping.get(domain, domain)  # 按映射改名，默认用原域名
    filepath = os.path.join("..", "data", f"{filename}.txt")
    # 记录时间到独立文件
    with open(filepath, "a") as file:
        file.write(f"{test_elapsed:.5f}\n")
...
def process_http_pushes(
    ...
    logger.info("run process_http_pushes")
...
def write_response(
    ...
    logger.info("run write_response")
...
async def main(
    ...
    async with connect(
        ...
        if parsed.scheme == "wss":
            ...
            # perform request
            logger.info("run main|url=%s|data=%s", urls, data)
            coros = [
