...
def calculate_success_distributed(send, recv, RTT, expect_time, test, loss, flag):
    ...
    for i in range(len(pros)):
        ...
        for j in range(0, i + 1):
            prob += pros[j]
        print(f"value<{i+1}RTT {prob}")
        with open(
            f"../data/test1/{test}_{loss}_{flag}_success_distributed.txt", "a"
        ) as file:
            print(f"value<{i+1}RTT {prob}", file=file)


# main
if __name__ == "__main__":
    loss = sys.argv[1]
    expect_time = float(sys.argv[2])
    RTT = int(sys.argv[3])
    flag = sys.argv[4]
    ...
    calculate_success_distributed(send, recv, RTT, expect_time, test, loss, flag)
