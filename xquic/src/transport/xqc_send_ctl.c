...
xqc_send_ctl_create(xqc_path_ctx_t *path)
{
    ...
    if (conn->conn_settings.enable_CCA_switching) {
        send_ctl->ctl_metric_cwnd_up_cb = conn->conn_settings.metric_cwnd_up_cb;
        send_ctl->ctl_metric_cwnd_down_cb = conn->conn_settings.metric_cwnd_down_cb;
        send_ctl->ctl_metric_ip_info_cb = conn->conn_settings.metric_cwnd_ip_info_cb;
        send_ctl->ctl_metric_sum_up_cb = conn->conn_settings.metric_cwnd_sum_up_cb;
        send_ctl->ctl_metric_free_resource_cb = conn->conn_settings.metric_free_resource_cb;
        send_ctl->ctl_metric_cwnd_up_thres_cb = conn->conn_settings.metric_cwnd_up_thres_cb;
        send_ctl->ctl_metric_cwnd_down_thres_cb = conn->conn_settings.metric_cwnd_down_thres_cb;
        send_ctl->ctl_metric_notice_can_evaluate_cb = conn->conn_settings.metric_notice_can_evaluate_cb;
        if (!(send_ctl->ctl_metric_cwnd_up_cb && send_ctl->ctl_metric_cwnd_down_cb 
            && send_ctl->ctl_metric_ip_info_cb && send_ctl->ctl_metric_sum_up_cb && send_ctl->ctl_metric_free_resource_cb
            && send_ctl->ctl_metric_cwnd_up_thres_cb && send_ctl->ctl_metric_cwnd_down_thres_cb)) {
            xqc_log(conn->log, XQC_LOG_ERROR, "|init cb failed|");
        }
    }
    
    /* added by jndu, init stream_CCA_info */
    memset(&send_ctl->stream_CCA_info, 0,
            sizeof(send_ctl->stream_CCA_info));
    send_ctl->stream_CCA_info.send_ctl = send_ctl;
    return send_ctl;
}
...
xqc_send_ctl_destroy(xqc_send_ctl_t *send_ctl)
{
    ...
    // free pcc proteus
    if (send_ctl->ctl_cong_callback->xqc_cong_ctl_destroy) {
        send_ctl->ctl_cong_callback->xqc_cong_ctl_destroy(send_ctl->ctl_cong);
    }
...
xqc_send_ctl_on_packet_sent(xqc_send_ctl_t *send_ctl, xqc_pn_ctl_t *pn_ctl, xqc_packet_out_t *packet_out, xqc_usec_t now)
{
    ...
    if (XQC_CAN_IN_FLIGHT(packet_out->po_frame_types)) {
        ...
        if (packet_out->po_pkt.pkt_pns == XQC_PNS_APP_DATA) {
            if (send_ctl->ctl_cong_callback->xqc_cong_ctl_on_packet_sent) {
                send_ctl->ctl_cong_callback->xqc_cong_ctl_on_packet_sent(send_ctl->ctl_cong, packet_out, now);
            }
        }
...
xqc_send_ctl_on_ack_received(xqc_send_ctl_t *send_ctl, xqc_pn_ctl_t *pn_ctl, xqc_send_queue_t *send_queue, xqc_ack_info_t *const ack_info, xqc_usec_t ack_recv_time, xqc_bool_t ack_on_same_path)
{
    ...
    xqc_list_for_each_safe(pos, next, &send_queue->sndq_unacked_packets[pns]) {
        ...
        if (packet_out->po_pkt.pkt_num >= range->low) {
            ...
            // added by jndu
            if (packet_out->po_pkt.pkt_pns == XQC_PNS_APP_DATA) {
                if (send_ctl->ctl_cong_callback->xqc_cong_ctl_on_ack_pcc) {
                    send_ctl->ctl_cong_callback->xqc_cong_ctl_on_ack_pcc(send_ctl->ctl_cong, packet_out, ack_recv_time, ack_recv_time - packet_out->po_sent_time);
                }
            }
    ...
    if (conn->conn_settings.enable_CCA_switching) {
        ...
        if (future_cong_ctrl) {
            void *future_cong = send_ctl->ctl_cong_stack[send_ctl->ctl_switch_ctx->ctx_future_CCA];
            ...
            if (future_cong_ctrl->xqc_cong_ctl_init_bbr /* && stream_frame_acked */) {
                ...
                */
                ...
                /* Make sure that we do not call BBR with a invalid sampler. */
                if (sample_type == XQC_RATE_SAMPLE_VALID) {
                        if ((future_cong_ctrl->xqc_cong_ctl_get_bandwidth_estimate != NULL)
                            && send_ctl->ctl_conn->log->log_level >= XQC_LOG_STATS
                            && (send_ctl->ctl_info.last_bw_time + send_ctl->ctl_info.record_interval <= now))
                        {
			...
			|| (lost_pn != XQC_MAX_UINT64_VALUE && po->po_pkt.pkt_num <= lost_pn))
		{
            ...
            if (po->po_flag & XQC_POF_IN_FLIGHT) {
                ...
                if(po->po_pkt.pkt_pns == XQC_PNS_APP_DATA) {
                    ...
                    if (send_ctl->ctl_cong_callback->xqc_cong_ctl_on_lost_pcc) {
                        send_ctl->ctl_cong_callback->xqc_cong_ctl_on_lost_pcc(send_ctl->ctl_cong, po, now);
                    }
                }
            ...
            if (conn->conn_settings.enable_CCA_switching) {
                ...
                if (future_cong_ctrl) {
                    if (future_cong_ctrl->xqc_cong_ctl_reset_cwnd) {
                        future_cong_ctrl->xqc_cong_ctl_reset_cwnd(send_ctl->ctl_cong_stack[send_ctl->ctl_switch_ctx->ctx_future_CCA]);
                    }
                }
            }
        }
    ...
    if (send_ctl->ctl_conn->conn_settings.enable_CCA_switching) {
        ...
        if (future_cong_ctrl) {
            if (future_cong_ctrl->xqc_cong_ctl_on_lost) {
                future_cong_ctrl->xqc_cong_ctl_on_lost(send_ctl->ctl_cong_stack[send_ctl->ctl_switch_ctx->ctx_future_CCA], sent_time);
            }
        }
    }
    ...
    if (send_ctl->ctl_conn->conn_settings.enable_CCA_switching) {
        ...
        if (future_cong_ctrl) {
            if (future_cong_ctrl->xqc_cong_ctl_on_ack) {
                future_cong_ctrl->xqc_cong_ctl_on_ack(send_ctl->ctl_cong_stack[send_ctl->ctl_switch_ctx->ctx_future_CCA], acked_packet, now);
            }
        }
    }
